---
title: MCP Server for Child Safety and Content Moderation
sidebarTitle: MCP
icon: plug
description: Connect Claude, Cursor or any MCP client to Tuteliq over OAuth. Around 60 tools for grooming detection, content moderation, incident review and compliance, with no API key to configure.
keywords: [mcp server, model context protocol, remote mcp, oauth 2.1, claude desktop, claude code, cursor, windsurf, content moderation tools, grooming detection, child safety api, streamable http, dynamic client registration, pkce]
---

Tuteliq runs a hosted [Model Context Protocol](https://modelcontextprotocol.io) server at `https://api.tuteliq.ai/mcp`. Point Claude Desktop, Claude Code, Cursor, Windsurf or any MCP-compatible client at that URL and sign in through your browser. There is nothing to install and no API key to paste into a config file.

It exposes around 60 tools covering grooming and bullying detection, image, voice and video analysis, fraud detection, incident review, age verification and compliance reporting. The same tool definitions back both the hosted server and the `@tuteliq/mcp` npm package (v3.19.0), so behaviour is identical whichever transport you use.

## Setup

Tuteliq is a hosted MCP server at `https://api.tuteliq.ai/mcp`. There is nothing
to install.

Most clients should connect with OAuth, which needs no credential in your
config at all. Two alternatives are documented below for cases OAuth cannot
cover: a static token for headless environments, and stdio for clients that do
not support remote servers.

### Connect with OAuth (recommended)

Point your client at the URL and sign in through the browser. Tuteliq
implements OAuth 2.1 with dynamic client registration and PKCE, so your client
discovers and registers itself. No key is pasted into a config file, the token
is scoped to your account, and it can be revoked from the dashboard without
editing anything on your machine.

<Steps>

<Step title="Add the server">
  **Claude Desktop:** open **Settings > Connectors**, click **Add custom
  connector**, name it **Tuteliq**, and set the URL to
  `https://api.tuteliq.ai/mcp`.

  **Claude Code, Cursor, Windsurf and other clients that support remote
  servers:** add the server to your `.mcp.json` with no credentials.
...
<Step title="Authenticate">
  Connect. Your browser opens a Tuteliq sign-in page; approve the connection and
  the client stores the token for you. In Claude Code, run `/mcp` to start the
  flow if it does not open on its own.
</Step>

</Steps>

Tuteliq tools are available in your next conversation. To revoke access later,
go to **Settings > Plugins** in the [dashboard](https://tuteliq.ai/dashboard).

### Static token (headless and automation)

OAuth needs a browser, so it cannot be completed by a CI pipeline, a cron job,
or a container with no interactive session. For those, send a token in the
`Authorization` header instead. Generate one in the
[dashboard](https://tuteliq.ai/dashboard) under **Settings > Plugins**.
...
<Warning>
  A static token in a config file is a long-lived credential. Keep it out of
  version control, and prefer OAuth anywhere a browser is available.
</Warning>

### stdio (clients without remote server support)

Some clients only speak stdio. The npm package runs a local process that calls
the same hosted API, so the tools are identical; only the transport and the
