...
for (const msg of result.messageAnalysis) {
  console.log(`Message ${msg.messageIndex}: risk=${msg.riskScore}, flags=${msg.flags}`)
}
...
Identify manipulation tactics designed to trick a child into disclosing information or taking unsafe actions. When you provide `conversationHistory`, the response includes a per-message risk breakdown — the same `messageAnalysis` field available on all conversation-aware endpoints.

```typescript
const result = await tuteliq.detectSocialEngineering({
  text: "All my real friends share their address. Don't you trust me?",
  ageGroup: '10-12',
  conversationHistory: [
    { role: 'stranger', text: "Hey, what school do you go to?" },
    { role: 'child', text: "Why do you want to know?" },
    { role: 'stranger', text: "If you really trusted me you'd send me your home address." },
  ],
})
...
// Per-message risk breakdown (available on all conversation-aware endpoints)
for (const msg of result.messageAnalysis) {
  console.log(`Message ${msg.messageIndex}: risk=${msg.riskScore}, flags=${msg.flags}`)
}
// Message 1: risk=0.4,  flags=["information_seeking"]
// Message 2: risk=0.1,  flags=[]
// Message 3: risk=0.85, flags=["trust_exploitation", "peer_pressure"]
```
