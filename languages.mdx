...
icon: globe
description: "Automatic language detection and culture-aware safety analysis in 32 languages: English stable, 31 beta"
keywords: [language detection, multilingual, i18n, localization, auto-detect]
---

Tuteliq detects and analyzes content in 32 languages. Language is auto-detected, with no configuration required.

**English is the only language tiered stable.** The other 31 are beta, which describes how much has been measured rather than whether detection runs: detection is enabled in every one of the 32, never sampled, gated or run at reduced escalation. Twenty-six of the beta languages are covered by a held-out in-language evaluation of 728 expert-pattern cases. Five (Chinese, Japanese, Korean, Arabic, Russian) have no evaluation of their own and rest on in-language smoke tests. Per-language precision does not exist for any beta language, because that evaluation holds no negatives. See the [vendor reference disclosure](/vendor-reference-disclosure) for the full position.
...
| `zh` | Chinese    | Beta   | Simplified and Traditional |
| `ja` | Japanese   | Beta   | |
| `ko` | Korean     | Beta   | |
| `ar` | Arabic     | Beta   | MSA plus Egyptian, Levantine, Gulf and Maghrebi dialects; Arabizi (Latin-and-numeral) writing recognized |
| `ru` | Russian    | Beta   | Translit (Latin-alphabet) writing recognized |
...
```json
{
  "error": {
    "code": "ANALYSIS_6009",
    "message": "Detected language \"Thai\" is not supported. Supported languages: en, es, pt, uk, sv, no, da, fi, de, fr, nl, pl, it, tr, ro, el, cs, hu, bg, hr, sk, lt, lv, et, sl, mt, ga, zh, ja, ko, ar, ru."
  }
}
```
