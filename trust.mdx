...
| **Retention** | No user content, ever. The only record kept is the end-to-end encrypted incident report, which is the model's response and contains no content. Opt out entirely with `incident_moderation_enabled: false`. |
...
One thing: the **incident report**. It is the model's response, not your content, and it is end-to-end encrypted. It exists so the moderation dashboard has something to show you, which is why it is **entirely optional**: set `incident_moderation_enabled: false` on the account or on any single call and nothing is written at all.
...
| Model rationale explaining the flag, stripped of quoted content and PII before it is written | Analysts need context to review an incident | 90 days, end-to-end encrypted, or never if disabled |
...
<Info>
  **End-to-end means we cannot read it.** Incident fields are encrypted to a public key you register, so only you can decrypt them. If you never register a key, or you disable incident logging, there is correspondingly less or nothing for anyone to read. Either way the underlying content was already gone.
</Info>
