...
Both low-severity findings were closed within two weeks: standardised
`RateLimit-*` response headers were added, and `'unsafe-inline'` was removed from
the script-src content security policy in favour of per-request nonces with
`strict-dynamic`. We also added an RFC 9116 `/.well-known/security.txt`. The
informational finding we did not action was the public exposure of the OpenAPI
specification, which is intentional: it is the documentation endpoint this site
is built from.
