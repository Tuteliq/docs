...
| Perceptual hash (`perceptual_hash` field) | **Returned in response, not used as cache key** | Computed by the forensic pipeline so you can match it against a known-material hash list, such as the [IWF Hash List](/synthetic-content#matching-against-the-iwf-hash-list) if your organisation is an IWF member. The cache itself is keyed on SHA-256. |
