...
Headless environment, or a client without remote server support? See [authentication](/mcp/authentication) for static tokens and stdio.
...
See the [full tool reference](/mcp/tools) for all ~80.
...
**Agents cannot resolve incidents.** An MCP token identifies an account, never an individual, so an agent is limited to `confirm` plus read-only work. Escalation, resolution and reopening stay with a human moderator. See [moderation](/mcp/moderation#what-an-agent-may-decide).
