...
### Fixed — `/account/consent` and `/account/audit-logs` 500s

Both endpoints returned `SVC_4001 An unexpected error occurred` when the user had multiple consent records or queried audit logs with the `action` filter. Root cause: missing Firestore composite indexes (`consent_records: consent_type + created_at`, `audit_logs: action + created_at`). Indexes are now in place — both endpoints return `200` again. These are the endpoints DPOs and SOC 2 auditors call first; restoring them was a Tier-1 unblock.

### Fixed — `/usage/monthly`, `/usage/history`, `/pricing/details` field-name mismatch

The three endpoints returned camelCase keys (`tierDisplayName`, `currentPeriodStart`, `totalRequests`, `priceMonthly`, …) while the SDK and the rest of the API surface use snake_case. SDK consumers reading `result.usage.percent_used` got `undefined`, and `undefined.toFixed()` crashed downstream. All three endpoints now return snake_case consistent with the broader API style.
