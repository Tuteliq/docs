...
## v3.1.0 (25 September 2026)

### Added: `reconnaissance` is returned per message

`POST /safety/grooming` now returns `reconnaissance` in `flags` on the individual
message that carries a supervision or availability probe. It was already part of the
published taxonomy, scored across a whole conversation; it is now also available at
message level, in any supported language and in informal register. The published
per-message taxonomy is therefore eight tactics rather than seven.

Two properties to design against before you switch on it.

**Safeguarding checks are deliberately not flagged.** A question that is only useful to
the asker if the answer is "yes, supervised" is a safeguarding check, not a probe, so a
tutor, coach or clinic confirming a guardian will be present does not produce this flag.
That behaviour is intended; please do not report it as a miss.

**One flag is not a pattern.** Accumulation across a conversation is a separate capability
and is not scored yet. Do not read a single `reconnaissance` as a completed profiling arc,
and do not read its absence across a conversation as the absence of probing.

`flags` remains a closed vocabulary, so callers switching on it will now see one additional
value. The published macro F1 of 0.987 was measured across the seven tactics that predate
this change and `reconnaissance` carries no per-tactic figure in that evaluation; see the
[vendor reference disclosure](/vendor-reference-disclosure).

## v3.0.0 (24 September 2026)

<Warning>
**Breaking.** `is_bullying` has changed meaning. If you branch on it to decide
whether to sanction the sender, read the first entry before upgrading.
</Warning>

### Changed: `is_bullying` reports whether bullying is present, not who did it

`is_bullying` now answers one question: is there a bullying signal in this
content. It is `true` for a child describing bullying being done to them,
exactly as it is for the person doing it.

Previously the field also encoded responsibility, so it returned `false`
whenever the speaker was the person being bullied rather than the one bullying.
That was at odds with how the field is defined, and with `speaker_role`, which
is documented as never changing it.

`speaker_role` is now the only field that separates the two, which is what it
was built for: `enacting` is the person bullying, `experiencing` is the person
being bullied, `reporting_other` is a bystander describing someone else.

**What to do.** If you use `is_bullying` to route content to a human, nothing
changes for you except that you will now see cases you were not seeing before.
If you use it to decide who to sanction, add a `speaker_role` check, or you
will act against the person who was harmed:

```js
// before
if (res.is_bullying) sanction(res.sender_id);

// after
if (res.is_bullying && res.speaker_role === 'enacting') sanction(res.sender_id);
if (res.is_bullying && res.speaker_role === 'experiencing') offerSupport(res.sender_id);
```

`severity`, `risk_score` and `recommended_action` are unchanged and were never
affected by the old behaviour. `recommended_action` remains the field we
recommend branching on for routing.

### Changed: a bullying disclosure is described as the speaker's own

Where a report of a specific, severe incident is surfaced for review and the
speaker is the person affected, the rationale now says so, rather than
describing it as a third-party report.

### Changed: a grooming verdict names the behaviour behind it

A `high` or `critical` grooming verdict that names no tactic is now returned as
`medium` with `recommended_action: flag_for_review`, and `risk_score` is brought
to the top of the medium band to match. The conversation still reaches a
reviewer; what it no longer does is raise an escalation with nothing attached
for that reviewer to check.

Findings that name even one tactic are untouched. Two new fields,
`verdict_capped` and `verdict_capped_reason`, report when this applied and why.

### Changed: an age stated in conversation is taken into account

Where a caller supplies a participant age and a participant states a different
age in the conversation itself, the analysis now uses the conversation. The
response reports this with `declared_age_overridden` and an explanation, so the
disagreement is visible at your end rather than resolved silently.
...
`/account/consent` and `/account/audit-logs` now respond reliably for accounts with larger or filtered datasets. Both endpoints are part of the GDPR-compliance observability surface (and the SOC 2 readiness work underway) and should be considered stable for compliance workflows.
