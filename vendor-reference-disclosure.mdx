...
Our grooming detection shows **zero false positives across 450 benign hard-negative turns** and per-tactic recall between 0.913 and 1.00 on a held-out expert corpus. Two things about that. The figures are self-measured by an interested party, which is exactly what this framework exists to fix. And precision is the metric most sensitive to base rate: carried to a realistic 1% turn-level prevalence it falls to roughly 0.54. Compute your own figure from your own prevalence rather than citing ours.
...
**What we corrected, and why it is worth stating.** An earlier version of this page described the evaluation as conversation-level, with 500 positive conversations against 27 negative ones, and reported precision 1.00 with a CI of 0.875 to 1.0. The underlying measurement was always turn-level. A reviewer had previously flagged a unit mismatch on this page, and the fix we applied re-based the figures onto 27 conversations, which moved the document further from what was actually measured rather than closer. An even earlier draft reported 0.992 to 1.0, which was nearer the truth than the "correction" that replaced it. We record this because a transparency document that quietly improves its own numbers is worth less than one that shows its working.
...
**Sextortion and coercive control are exercised but not separately measured.** They sit inside the same held-out expert evaluation (500 trajectories: 350 harmful, 150 benign hard negative), which spans grooming, sextortion, financial grooming, coercive control, image-based abuse and TFGBV, but the result is a macro across those harms and a macro is not a per-category precision and recall. An earlier draft of this page counted them as measured, which read as stronger than the evidence supports.
