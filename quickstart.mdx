...
<Step title="Make your first call">
  ...
  **Expected response:**
  ...
  <Warning>
    **`/safety/unsafe` is a starting point, not a complete safety layer.** It scores the KOSA
    harm categories: self-harm, eating disorders, substance use, depression and anxiety
    indicators, compulsive use, sexual content, violence and hate speech.

    It does not score everything a child-safety layer needs. Peer harassment, exposure or
    "fisha" accounts and image-based abuse of a named person are scored by
    [`/safety/bullying`](/bullying-toxicity-detection); grooming trajectories by
    `/safety/grooming`; extremist recruitment by `/safety/radicalisation`.

    It also returns `unsafe: false` when the sender is *describing* a harm rather than being
    subject to it: survivor testimony, a news article, a lesson, or a child relaying something
    they saw elsewhere. That is deliberate, and it keeps protective speech out of your review
    queue. It does mean a message another detector would flag can come back clear here.

    **So `unsafe: false` means "no KOSA-category signal on this endpoint", not "this content is
    safe."** Pair this endpoint with `/safety/bullying`, or call
    [`/analyse/multi`](#analyze-multiple-risks-at-once) and let one request cover several
    detectors.
  </Warning>

  <Tip>
    **Building a pipeline, not a UI?** The response above is designed to be read by a person — the full example includes fields like `rationale` meant for a moderator. If your code branches on `risk_score` / `severity` / `recommended_action` without rendering anything for a human, pass `options: { verdict_only: true }` (or `verdictOnly: true` in the SDKs) on any detection call. See [Fast mode](/fast-mode).
  </Tip>
</Step>
...
<Step title="Try fraud detection">
  ...
  <CodeGroup>
    ...
    ```
    ```bash cURL
    curl -X POST https://api.tuteliq.ai/api/v1/fraud/social-engineering \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "text": "I'\''m from your school'\''s IT department. I need your password to fix your account urgently.",
...
<Step title="Multi-endpoint analysis">
  ...
  <CodeGroup>
    ...
    ```
    ```bash cURL
    curl -X POST https://api.tuteliq.ai/api/v1/analyse/multi \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
...
<Tip>
  **What's next?** Explore the full [API Reference](/api-reference/introduction) or dive into one of our SDKs: [Node.js](/sdks/node), [Python](/sdks/python), [Swift](/sdks/swift), [Kotlin](/sdks/kotlin), [Flutter](/sdks/flutter), [React Native](/sdks/react-native), [.NET](/sdks/dotnet), [Unity](/sdks/unity), [CLI](/sdks/cli), or [MCP Server](/mcp-server).
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Fast mode" icon="bolt" href="/fast-mode">
    The recommended default for programmatic callers — structured fields without the human-facing formatting.
  </Card>
  ...
  <Card title="KOSA Coverage" icon="gavel" href="/kosa-compliance">
