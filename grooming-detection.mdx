...
| `gift_giving` | Offers of money, game currency, items or paid services to build obligation. Often framed as a favour with nothing expected in return. | *"I'll send you some Robux"*, *"don't worry about paying me back"* | Not separately categorised in KOSA or NCMEC schemas; reported within trust-building |
| `reconnaissance` | Probing who supervises the target and when they are unobserved: oversight of their device or person, and windows when they are alone. Includes probes phrased as statements. | *"Do your parents check your messages?"*, *"how late are you allowed out?"*, *"bet nobody checks on you"* | Not separately categorised in KOSA or NCMEC schemas; reported within predator targeting |

`reconnaissance` is the newest tactic in the taxonomy (added in V3.1.0, 25 September 2026). It was already part of the published taxonomy, scored across a whole conversation. It is now also returned on the individual message that carries the probe, in any of the supported languages and in informal register.

Two things about it are worth knowing before you switch on it. A question that is only useful if the answer is "yes, supervised" is a safeguarding check, not a probe, so a tutor or coach confirming a guardian will be present is deliberately **not** flagged. And a single `reconnaissance` flag is not a completed profiling pattern: accumulation across a conversation is a separate capability and is not scored yet, so do not read one flag as an arc, or the absence of flags as the absence of probing.

`meeting_request` was the newest tactic before that (added in V3.15.0); earlier versions of the detector subsumed it under `boundary_pushing`. We separated it because the legal and operational response to a meeting attempt is materially different from boundary pushing: most platforms route meeting indications to immediate human review and trigger external reporting workflows, not just internal moderation.
...
Two different limits apply, and it is worth keeping them apart.

**The schema limit is 100 messages.** A call carrying more than that is rejected by
request validation before any analysis runs, so it costs nothing and returns a
validation error rather than a verdict.

**The practical ceiling is around 50 turns.** Well inside the schema limit, long
conversations start to strain the engine's output budget. When the engine cannot produce
a verdict it returns a graceful envelope rather than a 500: `analysis_status:
"engine_error"`, a zeroed `risk_score`, and a forced `recommended_action:
"flag_for_review"`.

That envelope is the correct response, and the zeroed score is not a verdict. A moderator
must NOT read an engine error as "no grooming detected"; the forced
`flag_for_review` exists precisely so the case cannot be silently passed.

For conversations beyond roughly 30 turns, use chunked sliding windows of 10 to 15 turns
and pass the `continuation_token` from each call back into the next, so trajectory state
is preserved across windows without resending history. See
...
- [KOSA coverage](/kosa-compliance) — full mapping of detected tactics to KOSA harm categories.
